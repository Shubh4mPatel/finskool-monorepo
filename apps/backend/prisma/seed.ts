...
async function main() {
  ...
  const stocksSeedData: { name: string; symbol: string; sector: string; token: string | null; exchange: 'nse' | 'bse' }[] = [
    { name: 'Tata Steel Ltd', symbol: 'TATASTEEL', sector: 'Metals', token: '3499', exchange: 'nse' },
    { name: 'Adani Power Ltd', symbol: 'ADANIPOWER', sector: 'Energy', token: '17388', exchange: 'nse' },
    { name: 'Infosys Ltd', symbol: 'INFY', sector: 'IT', token: '1594', exchange: 'nse' },
    ...
    { name: 'Tata Motors Ltd', symbol: 'TATAMOTORS', sector: 'Auto', token: '3456', exchange: 'nse' },
    { name: 'ITC Ltd', symbol: 'ITC', sector: 'FMCG', token: '1660', exchange: 'nse' },
    { name: 'Wipro Ltd', symbol: 'WIPRO', sector: 'IT', token: '3787', exchange: 'nse' },
    { name: 'Bajaj Finance Ltd', symbol: 'BAJFINANCE', sector: 'NBFC', token: '317', exchange: 'nse' },
  ]

  for (const s of stocksSeedData) {
    await prisma.stock.upsert({
      where: { symbol: s.symbol },
      update: { name: s.name, sector: s.sector, token: s.token, exchange: s.exchange },
      create: { name: s.name, symbol: s.symbol, sector: s.sector, token: s.token, exchange: s.exchange },
    })
  }
